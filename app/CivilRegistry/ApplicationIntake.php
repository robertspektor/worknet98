...
class ApplicationIntake
{
    ...
    public function receive(Branch $office, WorldEvent $event, ApplicationKind $kind): ?WorkCase
    {
        $template = $this->templates->forKind($office->company, $kind);
        $application = $template === null ? null : $this->factory->create($office, $event, $kind);

        return $template === null || $application === null ? null : $this->opener->open($application, $template, $event);
    }
}
